...
export default async function AdminMatchesPage({ searchParams }: AdminMatchesPageProps) {
  ...
  const listMatches =
    plan.instantToggle || query.view === "list"
      ? sessionsInDateWindow(matches, plan.listRange)
      : [];
  const calendarMatches =
    plan.instantToggle || query.view === "calendar"
      ? sessionsInDateWindow(matches, plan.calendarRange)
      : [];
  const groups = groupMatchSessionsForParent(listMatches);
  const weekSessions = sessionsInWeek(calendarMatches, query.day);
        ...
        <PageHeader
          ...
          actions={
            <span className="flex flex-wrap gap-2">
              <Link href="/app/admin/matches/new" prefetch className={primaryButtonClassName}>
                {t("createMatch")}
              </Link>
