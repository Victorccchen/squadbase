...
type MatchCancelFormProps = {
  action: (prev: OrgActionState, formData: FormData) => Promise<OrgActionState>;
  confirmMessage: string;
};

export function MatchCancelForm({ action, confirmMessage }: MatchCancelFormProps) {
  ...
  return (
    ...
    >
      ...
      <button type="submit" disabled={pending} className={dangerButtonClassName}>
        {pending ? org("saving") : t("cancelMatch")}
      </button>
    </form>
  );
}
...
export function MatchRestoreForm({ action }: MatchRestoreFormProps) {
  const t = useTranslations("matches");
  const org = useTranslations("org");
  const [state, formAction, pending] = useActionState(action, INITIAL_ORG_ACTION_STATE);
  ...
  return (
    <form action={formAction} className="flex flex-col gap-2">
      ...
      <button type="submit" disabled={pending} className={secondaryButtonClassName}>
        {pending ? org("saving") : t("restoreMatch")}
      </button>
    </form>
  );
}
